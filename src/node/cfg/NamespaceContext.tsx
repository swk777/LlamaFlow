...
interface INamespaceContext {
  outside: TSFixMe;
  inside: TSFixMe;
  getValue(from: INamespaceFrom): TSFixMe;
  batchUpdateInside(fieldNames: string[], values: TSFixMe[]): void;
}
...
function genGetValue(nsObj) {
  return (from: INamespaceFrom): TSFixMe => {
...
const genContextValue = (outside, inside, setInside): INamespaceContext => {
  const batchUpdateInside = (fieldNames: string[], values: TSFixMe[]): void => {
