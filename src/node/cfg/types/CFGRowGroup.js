...
function CFGRowGroup (props) {
    ...
    const cDefs = configDefinitions.filter(getTargetCFGClass).filter(isDefVisible)
    const alignRight = align !== 'left'
    return (
        <div className={cn('row-flex-center', chooseCls('jc-end', '')(alignRight), className)} style={style}>
            {cDefs.map((cDef, i) => {
                const TargetDef = getTargetCFGClass(cDef)
                const cStyle = i !== 0 ? { marginLeft: 20, ...(cDef.style || {}) } : { ...(cDef.style || {}) }
                return (
                    <div key={cDef.fieldName} className="row-flex-center" style={cStyle}>
                        {cDef.label && <span className="no-shrink" style={{ marginRight: 8 }}>{cDef.label}</span>}
