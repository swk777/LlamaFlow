...
const getTitles = (name) => {
  switch (name) {
    ...
    case "com_guandata_plugin_spark_transform_calculator_CalculatorTransform": {
      return {
        title: fm({
          id: "ETL.CALC-FIELD-EDITOR",
          cn: "计算字段编辑器",
          en: "Calculated Field Editor",
        }),
        editorName: fm({
          id: "ETL.EXPRESSION",
          cn: "表达式",
        ...
        editorPlaceholder: fm({
          id: "ETL.CALC-FIELD-PHD",
          cn: "示例:`字段A`+`字段B` 请使用英文标点符号",
          en: "Example: Field A + Field B, Please use English punctuation",
        }),
      };
    }
    ...
    case "com_guandata_plugin_spark_transform_rowfilter_RowFilterTransform": {
      return {
        title: fm({
          id: "ETL.CONDITION-FILTER-EDITOR",
          cn: "过滤规则编辑器",
          en: "Condition filter editor",
        }),
        editorName: fm({ id: "RULES", cn: "规则", en: "Rules" }),
        editorPlaceholder: fm({
          id: "ETL.FILTER-FIELD-PHD",
          cn: "示例:`字段A`> 1 and `字段A` < 100 请使用英文标点符号",
          en: "Example: Field A > 1 and Field B < 100, Please use English punctuation",
        }),
      };
    }
    default:
...
function CFGFormulaEditor(props) {
  ...
  const [fieldValue, updateFv] = useDef(def);
  ...
  const isExprHasAggregatedFunc = useCallback(
    (expr) => {
  ...
  useEffect(() => {
    if (fieldValue) {
      batchUpdateInside(
        ...
        [isExprHasAggregatedFunc(fieldValue)]
      );
    }
  }, [fieldValue]);
    ...
    <div className="row-flex" style={{ width: "100%" }}>
      ...
      {editorOpen && (
        <ExprEditor
          ...
          dbType={
            outside.processType === PROCESS_TYPE.DB_DATAFLOW
              ? DB_TYPE.POSTGRESQL
              : DB_TYPE.SPARK
          }
