...
const MemoVirtualItem = React.memo(({ index, style }) => {
  const {
    values,
    move,
    readonly,
      ...
      {definitions.map((def) => {
        ...
        return (
          <div
            key={key}
            style={divStyle}
            className={cn("row-flex-center text-left weight-600", cls)}
          >
            {def.label}
...
const sortByPosition = (a, b) =>
  (a.displayPosition || 0) - (b.displayPosition || 0);

function CFGListBean(props) {
  const { definition } = props;
  let itemHeight = 42;
  if (_get(definition, "model.configDefinitions[0].fieldName") === "sql")
    itemHeight = 90;
  const {
    configDefinitions,
    initValue = genInitValue(configDefinitions),
    hideHeader,
    sortEnabled,
  } = definition.model;
  const sortedDefinitions = useMemo(
    () => configDefinitions.sort(sortByPosition),
    [configDefinitions]
  );

  const [fieldValue, updateFv, readonly] = useDef(definition);
  const [values, setValues] = useState(() =>
    (fieldValue || []).map(addIndexKey)
  );
        ...
        <div className="column-flex flex1">
          {!hideHeader && (
            <Fragment>
              <BeanHeader
                definitions={cDefs}
                style={{ height: 20, paddingRight: 54 }}
                sortable={sortEnabled}
              />
          ...
          >
            <VirtualList
              width="100%"
              height={
                values.length > APPEAR_SCROLL_NUM
                  ? itemHeight * APPEAR_SCROLL_NUM
