...
export const updateIndex = (index, val) => (vals) =>
  vals.map((v, i) => (i === index ? val : v));
...
export function AddIcon(props) {
  ...
  return (
    <Icon
      name="add_item color-text-icon-default ml-8"
      onClick={readonly ? null : addNew}
      style={style}
    />
  );
}
...
function CFGList(props) {
  ...
  const {
    ...
    initValue = undefined,
    sortEnabled,
  } = definition.model || genFallbackModel(definition);
  ...
  const [values, setValues] = useState(() =>
    (fieldValue || []).map((value) => addIndexKey({ value }))
  );

  const update = useCallback(() => {
    setValues((fieldValue || []).map((value) => addIndexKey({ value })));
  }, [fieldValue]);
  ...
  const addNew = () =>
    !readonly && updateValue(values.concat(addIndexKey({ value: initValue })));
  ...
  const targetDef = useMemo(
    () => ({ ...configDefinition, fieldName: "value" }),
    [configDefinition]
  );
