...
function CFGColumnSelector(props: IProps): ReactElement {
  ...
  const [values, setValues] = useState(
    (fieldValue || []).filter((v) => v.colName).map(addIndexKey)
  );
  ...
  const missFields = useMemo(
    () =>
      values
        .map((v) => ({ ...v.colName, missing: true }))
        .filter((v) => labels.indexOf(v.name) === -1),
    [labels, values]
  );
  ...
  const columns = useMemo(() => {
    if (debounceSearchValue) {
      try {
        const cols = labels
          .map((v) => ({ name: v, type: getType(v) }))
          .filter((v) => {
            const index = values.findIndex((val) => getColName(val) === v.name);
            if (index !== -1) {
              return (
                values[index].newColName || values[index].colName.name
              ).match(safeNewRegExp(debounceSearchValue, "i"));
            }
    ...
    return missFields
      .concat(labels.map((v) => ({ name: v, type: getType(v) })))
      .map(addIndexKey);
  ...
  const handleSelectAll = (e): void => {
    ...
    if (checked) {
      ...
      updateValue(
        values.filter(
          (v) => columns.findIndex((col) => col.name === getColName(v)) === -1
        )
      );
    }
  };
  const handleSwitchType = (): void => {
    if (readonly) return;
    if (!isBulkEdit && fieldValue.length === 0) {
      updateFv([{}]);
    }
    if (isBulkEdit) {
  ...
  const handleAntiSelect = (): void => {
    ...
    // 剔除选中的
    const selectedItems = values.filter(
      (v) =>
        newValues.findIndex((val) => getColName(val) === getColName(v)) !== -1
    );
  ...
  }, [fieldValue, isBulkEdit]);
  useEffect(() => {
    setValues((fieldValue || []).filter((v) => v.colName).map(addIndexKey));
  }, [(fieldValue || []).length]);
  ...
  const isAllSelected =
    ...
    !!values.length &&
    columns.every(
      (col) => values.findIndex((v) => getColName(v) === col.name) !== -1
    );
  ...
  return (
    <div className={cn(s.container, "flex1")}>
      ...
      {isBulkEdit && (
        <CFGBulkEditor definition={definition} missFields={missFields} />
      )}
      <div className="row-flex-center jc-space" style={{ marginTop: 10 }}>
        ...
        {definition.bulkEdit && (
          <Text
            className={
              readonly
                ? "color-text-disable cursor-not-allowed"
                : "cursor-pointer color-text-info"
            }
