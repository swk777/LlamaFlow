...
import DefContext from '../../cfg/DefContext'
import { validateRequired } from '../../cfg/validation'
import { validate } from '../../cfg-scheduler/model/validation'

import fm from '../../../core/i18n'

const MSG_NOT_CONFIGURED = { id: 'NOT_CONFIGURED', cn: '节点未配置', en: 'Configuration is not complete' }
const getFieldName = (def): string => _get(def, 'model.relatedRequiredFieldName') || def.fieldName
...
function CFGStatus (props: IProps): ReactElement {
    ...
    if (_get(failedDefs, 'length') === 0 && isValidConfig(def, value, config)) return null
