  ...
  return (
    <Fragment>
      <Input
        placeholder={phK}
        value={value.key}
        onChange={genChange("key")}
        disabled={disabled}
      />
      <Input
        placeholder={phV}
        value={value.value}
        onChange={genChange("value")}
        disabled={disabled}
      />
    </Fragment>
  );
...
function CFGMap(props) {
  ...
  return (
    <div
      key={values.length}
      className={cn("row-flex", className)}
      style={style}
    >
      <TitleClass
        def={def}
