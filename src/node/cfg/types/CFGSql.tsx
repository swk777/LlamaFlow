...
import { INVALID_PARAMS, useFetch, useVersion, useVisible } from '@guandata/hooks'
...
import { ICFGBaseProps, ICFGDefinitionBase } from '../../../types/cfgDefinition'
import useDef from '../useDef'
import { getSqlEditorTitlesBySqlCategory, SqlCategory } from '../../../constants/sql'
...
function SqlEditModal ({ visible, datasetId, sqlCategory, disabled, ...rest }: IModalProps): ReactElement {
    const [ res, loading, setParams ] = useFetch(getDatasetDetail, [], INVALID_PARAMS)
    ...
    const titles = useMemo(() => getSqlEditorTitlesBySqlCategory(sqlCategory, fm), [ sqlCategory ])
...
export default function CFGSql ({ definition: def, style, className }: ICFGBaseProps<ICFGSql>): ReactElement {
    useStyles(s)
    ...
    const updateByModal = useCallback(v => { updateFv(v); upgradeVersion() }, [ updateFv, upgradeVersion ])
    return (
        <div style={style} className={cn('pos-relative', className)}>
            ...
            />
            {datasetId && (
                <div className={cn(s.editToolbar, 'pos-absolute color-background-alt shadow-popover-down')}>
                    <Button onClick={showEdit} icon="fg fg-pannelRename" size="small" noBorder>
                        {fm(MsgEdit)}
                    </Button>
                </div>
            )}
