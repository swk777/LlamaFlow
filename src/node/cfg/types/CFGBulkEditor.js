...
function CFGBulkEditor (props) {
    ...
    const values = preValues(fv.filter(v => v.colName && missFields.every(m => m.name !== v.colName.name)))
    ...
    return (
        <div className="column-flex flex1">
            <CodeEditor
                ...
                className="font-m"
                supportSenior
                style={{ width: '100%', minHeight: 120, maxHeight: 120, overflow: 'hidden' }}
            />
            {showError && <Text className="color-text-destructive">{'json格式异常'}</Text>}
        </div>
    )
}
